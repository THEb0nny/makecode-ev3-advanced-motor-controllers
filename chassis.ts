...
namespace chassis {
    ...
    let brakeSettleTime = 50; // Время для стабилизации после тормоза шасси в мсек
    ...
    */
    ...
    /**
     * Установить время стабилизации тормоза шасси после торможения в мсек.
     * При слишком маленьком времени могут появиться проблемы с функциями регулирования шасси. Например, робот не сможет повернуться на заданный угол.
     * Значение settleTime всегда должны быть положительными (отрицательные значения будут взяты по модулю).
     * @param settleTime время стабилизации шасси в мсек, eg: 50
     */
    ...
    */
    ...
    export function syncMovement(vLeft: number, vRight: number, value: number, unit: MoveUnit, braking: MotionBraking) {
        ...
        while (true) { // Цикл синхронизации движения
            ...
            const errorRaw = advmotctrls.getErrorSyncMotors(eml, emr, vLeft, vRight); // Найдите ошибку в управлении двигателей
    ...
    /**
     * Синхронизация двигателей в шасси с установкой скоростей (мощностей) для каждого двигателя.
     * @param turnRatio рулевой параметр, если больше 0, то поворачиваем вправо, а если меньше, то влево, eg: 0
